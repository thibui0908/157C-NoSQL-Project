...
class Billing:
    ...
    def __init__(self, film_title, purchase_date, due_date):
...
def addBilling(user_id, film_title):
    ...
    newBilling = Billing(film_title, purchase_date.strftime(
        "%d-%m-%Y"), due_date.strftime("%d-%m-%Y"))
    
    billingDict = {"film_title" : film_title, purchase_date: str(purchase_date), due_date: str(due_date)}
